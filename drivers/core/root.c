...
static int dm_scan_fdt_node(struct udevice *parent, ofnode parent_node,
  ...
  for (node = ofnode_first_subnode(parent_node); ofnode_valid(node);
...
__weak int dm_scan_other(bool pre_reloc_only) { return 0; }
...
static int dm_probe_devices(struct udevice *dev, bool pre_reloc_only) {
  ...
  if (pre_reloc_only && (!ofnode_valid(node) || !ofnode_pre_reloc(node)) &&
...
int dm_init_and_scan(bool pre_reloc_only) {
  ...
  if (CONFIG_IS_ENABLED(DM_EVENT)) {
    ret = event_notify_null(gd->flags & GD_FLG_RELOC ? EVT_DM_POST_INIT_R
                                                     : EVT_DM_POST_INIT_F);
    if (ret)
...
void dev_collect_stats(struct dm_stats *stats, const struct udevice *parent) {
  ...
  for (i = 0; i < DM_TAG_ATTACH_COUNT; i++) {
    ...
    if (size || (i == DM_TAG_DRIVER_DATA && parent->driver_data)) {
...
U_BOOT_DRIVER(root_driver) = {
    .name = "root_driver", .id = UCLASS_ROOT, ACPI_OPS_PTR(&root_acpi_ops)};
